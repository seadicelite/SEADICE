...
App Store登録用テキストは**まず日本語のみ**生成する（多言語対応は行わない）：
- アプリ名（30文字以内）
- サブタイトル（30文字以内）
...
英語版は開発完了後、Step 6.5でまとめて生成する（| en | 英語 | English (U.S.) |）。開発中に日英を都度セットで作るとイテレーションのたびに翻訳コストがかかるため。
...
**assets/i18n/** に**まず ja.json のみ**作成する（en.json は作らない）：
...
## Step 6.6: 英語翻訳の一括生成（リリース直前・1回だけ）
